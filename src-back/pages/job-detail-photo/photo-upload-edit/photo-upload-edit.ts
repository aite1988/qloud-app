...
import { PhotoDrawPage } from '../photo-draw/photo-draw';


@Component({
  selector: 'page-photo-upload-edit',
  templateUrl: 'photo-upload-edit.html'
})
export class PhotoUploadEditPage {
...
selectImage(){
  ...
  cropPhoto() {
    let croppedImgB64String: string = this.angularCropper.cropper.getCroppedCanvas().toDataURL('image/jpeg', (50/ 100));
...
//ไปหน้าวาดเขียน
  //this.navCtrl.push(PhotoDrawPage,{photo:this.photo,app_id:this.app_id,name:this.name});
  this.uploadPhoto();
  }
  ...
  uploadPhoto()
{
  ...
  let postDat={app_id:this.app_id,name:this.name,photo:this.photo,photo_type:"image/jpeg"}
  console.log(postDat);
